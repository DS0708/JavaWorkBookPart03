...
- 쿠키를 주고받는 기본적인 시나리오
  ...
  <img src="./image/setCookie.png" width="400" height="400">
...
> 이때 Tomcat은 JSESSIONID라는 쿠키의 value를 세션 저장소(Session Repository)에 key형태로 
> 저장하며 사용자별 정보를 저장한다.
...
```
- doGet()
  - TodoRegisterController를 통해 "/login"으로 Redirect되면 login.jsp를 불러온다
- doPost()
  - 사용자의 mid와 mpw를 수집하여 HttpSession 공간에 "loginInfo"라는 이름(key)으로 저장한다.
